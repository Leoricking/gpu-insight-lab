...
GPU Insight Lab 是一個可用於面試展示的 GPU 工程作品集專案。它展示的不只是會寫 CUDA kernel，
而是能把 PCIe 傳輸分析、benchmark 方法論、profiling 整合、正確性驗證、證據式診斷、版本回歸比較
與專業報告輸出整合成完整工程流程。

一般 CUDA sample 只能證明會寫 kernel；GPU Insight Lab 要展示的是能建立可重複、可驗證、
可報告的 GPU software engineering workflow。

**目標職缺**：CUDA Performance Engineer · GPU Software Engineer · HPC Validation Engineer ·
GPU PCIe / System Validation · NVIDIA / AMD GPU Platform Software

面試準備、技術論點佐證、職缺對齊作品集專案，以及完整 CUDA 題庫，請參閱
[`docs/INTERVIEW_GUIDE.md`](docs/INTERVIEW_GUIDE.md)。

12 週 CUDA → NVIDIA / AMD 求職路線圖，請參閱
[`docs/12_WEEK_CUDA_JOB_ROADMAP.md`](docs/12_WEEK_CUDA_JOB_ROADMAP.md)。
